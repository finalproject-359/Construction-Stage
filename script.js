...
const exportDashboardReport = (format = "xlsx") => {
  ...
  if (format === "csv") {
    ...
    `;
    const dateRangeHtml = `<p class="pdf-date-range"><strong>Date Range:</strong> ${escapeHtml(resolvedDateRange)}</p>`;
    ...
    if (printWindow) {
      ...
      printWindow.document.write(`<html><head><title>${filenameBase}</title><style>${pageCss} body{font-family:Inter,Arial,sans-serif;padding:0;font-size:${pdfPrefs.fontSize};color:#0f172a;background:#f8fafc;} .pdf-shell{padding:138px 0 96px;} .pdf-content{padding:0 22px;} .pdf-card{background:#fff;border:1px solid #dbe3f2;border-radius:10px;padding:10px 12px;margin:0 0 12px;box-shadow:0 1px 0 rgba(15,23,42,.04);} .pdf-report-header,.pdf-report-footer{position:fixed;left:0;right:0;color:#fff;background:linear-gradient(90deg,#021634 0%,#02224f 50%,#011530 100%);} .pdf-report-header{top:0;height:108px;padding:14px 24px;display:flex;align-items:center;justify-content:space-between;border-bottom:4px solid #d6a45b;} .pdf-header-left{display:flex;align-items:center;gap:14px;} .pdf-logo{height:54px;object-fit:contain;} .pdf-header-divider{width:1px;height:54px;background:rgba(214,164,91,.7);} .pdf-title-wrap h1{margin:0;font-size:24px;line-height:1.2;color:#fff;} .pdf-title-wrap p{margin:6px 0 0;font-size:15px;color:#e2e8f0;} .pdf-header-right p{margin:0;font-size:14px;color:#f8fafc;} .pdf-date-range{margin:0 0 18px 0;font-size:16px;} .pdf-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px;} .pdf-metric{border:1px solid #e2e8f0;border-radius:8px;padding:8px;background:#f8fafc;} .pdf-metric span{display:block;font-size:10px;color:#475569;text-transform:uppercase;letter-spacing:.04em;} .pdf-metric strong{font-size:13px;color:#0f172a;} .pdf-report-footer{bottom:0;height:74px;padding:10px 24px;border-top:4px solid #d6a45b;display:flex;align-items:center;justify-content:space-between;} .pdf-footer-block{display:flex;align-items:center;gap:14px;} .pdf-footer-divider{width:1px;height:42px;background:rgba(214,164,91,.65);} .pdf-footer-logo{height:40px;object-fit:contain;} .pdf-footer-text{font-size:14px;line-height:1.35;color:#f8fafc;} .pdf-warning-list{margin:0;padding-left:18px;} table{border-collapse:separate;border-spacing:0;width:100%;font-size:inherit;background:#fff;border:1px solid #dbe3f2;border-radius:10px;overflow:hidden;} th,td{border-bottom:1px solid #e2e8f0;padding:7px 8px;} th{background:#f1f5f9;color:#0f172a;text-transform:none;font-size:inherit;letter-spacing:0;font-weight:700;} tr:nth-child(even) td{background:#f8fafc;} tr:last-child td{border-bottom:none;} h3{margin:0 0 8px;font-size:14px;color:#1e293b;}</style></head><body><div class="pdf-shell"><div class="pdf-content">${bodyParts}</div></div><footer class="pdf-report-footer"><div class="pdf-footer-block"><img src="${logoPath}" alt="CosTrack logo" class="pdf-footer-logo" /></div><div class="pdf-footer-divider" aria-hidden="true"></div><div class="pdf-footer-text">www.costrack.com<br/>info@costrack.com</div><div class="pdf-footer-divider" aria-hidden="true"></div><div class="pdf-footer-text">Page 1 of 1<br/>${safeGeneratedAt}</div></footer></body></html>`);
