...
const exportSettingsModalEl = document.getElementById("exportSettingsModal");
...
const pdfIncludeDateRangeEl = document.getElementById("pdfIncludeDateRange");
...
const EXPORT_COLUMNS = [
  ...
  { key: "Finish Date", label: "Finish Date" },
  { key: "Planned Cost", label: "Planned Cost" },
  { key: "Actual Cost", label: "Actual Cost" },
  { key: "Earned Value", label: "Earned Value" },
  { key: "Cost Variance", label: "Cost Variance" },
  { key: "Percent Complete", label: "Percent Complete" },
  { key: "Cost Used %", label: "Cost Used %" },
];
...
const getSelectedExportColumns = () => {
  try {
    ...
    const valid = parsed.filter((key) => EXPORT_COLUMNS.some((column) => column.key === key));
    return valid.length ? valid : EXPORT_COLUMNS.map((column) => column.key);
  } catch {
    return EXPORT_COLUMNS.map((column) => column.key);
  }
};

const applyExportColumnSelection = (rows) => {
...
const buildExportWarnings = (rows) => {
  const warnings = [];
  const missingDateCount = rows.filter((row) => !normalizeDateOnly(row.startDate) && !normalizeDateOnly(row.finishDate)).length;
  const missingCostCount = rows.filter((row) => !Number.isFinite(parseNumber(row.plannedCost)) || !Number.isFinite(parseNumber(row.actualCost))).length;
  if (missingDateCount) warnings.push(`Rows with missing start/finish dates: ${missingDateCount}`);
  if (missingCostCount) warnings.push(`Rows with missing/invalid planned or actual cost: ${missingCostCount}`);
  return warnings.length ? warnings : ["No data quality warnings detected."];
};
...
const openPdfSettingsModal = () => {
  ...
  if (pdfIncludeDateRangeEl) pdfIncludeDateRangeEl.checked = !!prefs.includeDateRange;
...
const exportDashboardReport = (format = "xlsx") => {
  ...
  const selectedRows = applyExportColumnSelection(exportRows);
  const warnings = buildExportWarnings(filteredRows);
  const now = new Date();
  const dateStamp = now.toISOString().slice(0, 10);
  const timeStamp = now.toISOString().slice(11, 16).replace(":", "");
  const projectLabel = projectFilterEl?.value || "all";
  const dateRangeLabel = dateRangeFilterEl?.value || "all";
  const filenameBase = `costrack-dashboard-report-${dateStamp}_${timeStamp}_project-${slugify(projectLabel)}_date-${slugify(dateRangeLabel)}`;
  const totals = calculateTotalsFromRows(filteredRows);
  const progress = calculateProgressMetrics(filteredRows, totals);
  const metadataRows = [
    { Field: "Generated At (UTC)", Value: now.toISOString() },
    { Field: "Generated At (Local)", Value: now.toLocaleString() },
    { Field: "Project Filter", Value: projectLabel },
    ...
    { Field: "Date Start", Value: dateStartFilterEl?.value || "-" },
    { Field: "Date End", Value: dateEndFilterEl?.value || "-" },
  ];
  ...
  if (format === "csv") {
    ...
    const resolvedDateRange = (() => {
      const start = dateStartFilterEl?.value || "";
      const end = dateEndFilterEl?.value || "";
      if (dateRangeLabel === "custom") return `${start || "-"} to ${end || "-"}`;
    ...
    const header = `
      <header class="pdf-report-header">
        <div class="pdf-header-left">
          <img src="${logoPath}" alt="CosTrack logo" class="pdf-logo" />
          <div class="pdf-header-divider" aria-hidden="true"></div>
          <div class="pdf-header-copy">
            <h1>CosTrack Dashboard Report</h1>
            <p>Project: ${safeProjectLabel}</p>
          </div>
        </div>
        <div class="pdf-header-right">
          <p>Generated: ${safeGeneratedAt}</p>
          <span class="pdf-header-icon">📅</span>
        </div>
      </header>
    `;
    ...
    if (printWindow) {
      ...
      const bodyParts = [
        ...
        pdfPrefs.includeDateRange ? dateRangeHtml : "",
        pdfPrefs.includeSummary ? summary : "",
        pdfPrefs.includeWarnings ? warningHtml : "",
        pdfPrefs.includeTable ? `<table>${tableHead}${tableRows}</table>` : "",
      ].join("");
      printWindow.document.write(`<html><head><title>${filenameBase}</title><style>${pageCss} body{font-family:Inter,Arial,sans-serif;padding:0;font-size:${pdfPrefs.fontSize};color:#0f172a;background:#f8fafc;} .pdf-shell{padding:118px 0 86px;} .pdf-content{padding:0 10px;} .pdf-card{background:#fff;border:1px solid #dbe3f2;border-radius:10px;padding:10px 12px;margin:0 0 12px;box-shadow:0 1px 0 rgba(15,23,42,.04);} .pdf-report-header{position:fixed;top:0;left:0;right:0;min-height:94px;border-bottom:3px solid #ca8a04;background:linear-gradient(120deg,#03152f 0%,#022b56 45%,#001733 100%);padding:12px 18px;color:#fff;display:flex;align-items:center;justify-content:space-between;gap:14px;} .pdf-header-left{display:flex;align-items:center;gap:12px;} .pdf-logo{height:54px;width:auto;object-fit:contain;} .pdf-header-divider{width:1px;height:56px;background:rgba(255,255,255,.65);} .pdf-header-copy h1{margin:0;font-size:20px;line-height:1.2;color:#fff;} .pdf-header-copy p{margin:6px 0 0;font-size:14px;color:#e2e8f0;} .pdf-header-right{display:flex;align-items:center;gap:10px;font-size:13px;color:#e2e8f0;text-align:right;} .pdf-header-icon{border:2px solid #ca8a04;color:#fcd34d;width:36px;height:36px;border-radius:999px;display:inline-flex;align-items:center;justify-content:center;font-size:16px;} .pdf-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px;} .pdf-metric{border:1px solid #e2e8f0;border-radius:8px;padding:8px;background:#f8fafc;} .pdf-metric span{display:block;font-size:10px;color:#475569;text-transform:uppercase;letter-spacing:.04em;} .pdf-metric strong{font-size:13px;color:#0f172a;} .pdf-report-footer{position:fixed;left:0;right:0;bottom:0;height:68px;border-top:3px solid #ca8a04;background:linear-gradient(120deg,#03152f 0%,#022b56 45%,#001733 100%);padding:8px 18px;display:flex;justify-content:space-between;align-items:center;font-size:11px;color:#e2e8f0;} .pdf-footer-left,.pdf-footer-center,.pdf-footer-right{display:flex;align-items:center;gap:8px;} .pdf-footer-logo{height:34px;width:auto;object-fit:contain;} .pdf-footer-sep{width:1px;height:34px;background:rgba(255,255,255,.55);} .pdf-footer-icon{border:2px solid #ca8a04;color:#fcd34d;width:28px;height:28px;border-radius:999px;display:inline-flex;align-items:center;justify-content:center;} .pdf-footer-right{font-size:12px;font-weight:600;} .pdf-warning-list{margin:0;padding-left:18px;} table{border-collapse:separate;border-spacing:0;width:100%;font-size:inherit;background:#fff;border:1px solid #dbe3f2;border-radius:10px;overflow:hidden;table-layout:fixed;} th,td{border-bottom:1px solid #e2e8f0;padding:7px 8px;word-break:break-word;overflow-wrap:anywhere;white-space:normal;vertical-align:top;} th{background:#e2e8f0;color:#1e293b;font-size:10px;letter-spacing:.04em;} tr:nth-child(even) td{background:#f8fafc;} tr:last-child td{border-bottom:none;} h3{margin:0 0 8px;font-size:14px;color:#1e293b;} @media print{table,thead,tbody,tr,th,td{page-break-inside:avoid;break-inside:avoid;}}</style></head><body><div class="pdf-shell"><div class="pdf-content">${bodyParts}</div></div><footer class="pdf-report-footer"><div class="pdf-footer-left"><img src="${logoPath}" alt="CosTrack logo" class="pdf-footer-logo" /></div><span class="pdf-footer-sep" aria-hidden="true"></span><div class="pdf-footer-center"><span class="pdf-footer-icon">🌐</span><span>www.costrack.com<br/>info@costrack.com</span></div><span class="pdf-footer-sep" aria-hidden="true"></span><div class="pdf-footer-right"><span class="pdf-footer-icon">📄</span><span>Page 1 of 1<br/>${safeGeneratedAt}</span></div></footer></body></html>`);
    ...
    const summarySheet = XLSX.utils.json_to_sheet([
      ...
      { Metric: "Total Earned Value", Value: totals.ev },
      { Metric: "Total Cost Variance", Value: totals.cv },
      { Metric: "Physical Progress %", Value: progress.physicalProgressPercent / 100 },
      { Metric: "Cost Spent %", Value: progress.costSpentPercent / 100 },
...
if (exportReportButtonEl && exportReportOptionsEl) {
  ...
  exportReportOptionsEl.addEventListener("click", (event) => {
    ...
    if (actionButton && exportSettingsModalEl) {
      const selected = new Set(getSelectedExportColumns());
      exportColumnListEl.innerHTML = EXPORT_COLUMNS.map((column) => `
        <label><input type="checkbox" value="${column.key}" ${selected.has(column.key) ? "checked" : ""}/> ${column.label}</label>
      `).join("");
      exportSettingsModalEl.hidden = false;
    }
...
if (exportSettingsSaveEl && exportSettingsCancelEl && exportSettingsModalEl) {
  exportSettingsCancelEl.addEventListener("click", () => {
    exportSettingsModalEl.hidden = true;
  });
  exportSettingsSaveEl.addEventListener("click", () => {
    const selected = Array.from(exportColumnListEl.querySelectorAll("input[type='checkbox']:checked")).map((el) => el.value);
...
if (pdfSettingsModalEl && pdfSettingsCancelEl && pdfSettingsPrintEl) {
  ...
  pdfSettingsPrintEl.addEventListener("click", () => {
    const prefs = {
      ...
      includeDateRange: !!pdfIncludeDateRangeEl?.checked,
